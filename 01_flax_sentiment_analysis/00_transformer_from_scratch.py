...
class SelfAttention(fnn.Module):
    ...
    def __call__(self, v, k, q):
        ...
        v = v.reshape(n, value_len, self.n_heads, head_dim)
        k = k.reshape(n, key_len, self.n_heads, head_dim)
        q = q.reshape(n, query_len, self.n_heads, head_dim)
...
class TransformBlock(fnn.Module):
    ...
    @fnn.compact
    def __call__(self, value, key, query, *, train):
